...
Copyright © 2025 AZ1.AI INC. All Rights Reserved."""
    ...
    def install_dependencies(self) -> bool:
        """Install Python dependencies via cross-platform install.py"""
        print(f"\n{Colors.BLUE}{Colors.BOLD}▶ Installing Python dependencies (venv + GitPython)...{Colors.END}\n")

        coditect_dir = WORKSPACE_DIR / ".coditect"
        install_script_py = coditect_dir / "scripts" / "installer" / "install.py"
        install_script_sh = coditect_dir / "scripts" / "installer" / "install.sh"

        try:
            # Prefer cross-platform Python installer
            if install_script_py.exists():
                print(f"{Colors.CYAN}Using cross-platform installer (install.py)...{Colors.END}\n")
                result = subprocess.run(
                    [sys.executable, str(install_script_py)],
                    cwd=coditect_dir,
                    capture_output=False,  # Show output to user
                    text=True
                )
            elif install_script_sh.exists():
                print(f"{Colors.CYAN}Using bash installer (install.sh)...{Colors.END}\n")
                result = subprocess.run(
                ...
                print(f"{Colors.YELLOW}ℹ No installer found, skipping dependency installation{Colors.END}")
                print(f"{Colors.YELLOW}  Run manually: python3 scripts/installer/install.py{Colors.END}")
                return True
            ...
            if result.returncode == 0:
                ...
                print(f"\n{Colors.YELLOW}⚠ Dependency installation had warnings (non-critical){Colors.END}")
                return True  # Don't fail setup for dependency issues

        except Exception as e:
            print(f"{Colors.YELLOW}⚠ Could not install dependencies: {e}{Colors.END}")
            print(f"{Colors.YELLOW}  You can run 'python3 scripts/installer/install.py' manually later{Colors.END}")
