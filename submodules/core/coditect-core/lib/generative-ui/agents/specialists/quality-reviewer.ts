 ...
 */
export class QualityReviewer extends BaseAgent<QualityReviewInput, QualityReport> {
  constructor() {
    super({
      name: 'quality-reviewer',
      role: AgentRole.VALIDATOR,
      description:
        'Ensures production-ready React + TypeScript code quality and best practices',
      capabilities: [
  ...
  async execute(
  ...
  ): Promise<AgentResult<QualityReport>> {
    ...
    try {
      ...
      const validation: ValidationResult[] = report.issues.map((issue) => ({
        rule: issue.rule,
        passed: false,
   ...
   */
  private async reviewQuality(input: QualityReviewInput): Promise<QualityReport> {
   ...
   */
  private reviewTypeScript(code: string): {
    issues: QualityIssue[];
    strengths: string[];
  } {
    const issues: QualityIssue[] = [];
   ...
   */
  private reviewReactPatterns(code: string): {
    issues: QualityIssue[];
    strengths: string[];
  } {
    const issues: QualityIssue[] = [];
    ...
    if (code.includes('useState') || code.includes('useEffect')) {
      ...
      if (code.includes('useEffect') && !code.match(/return\s*\(\s*\)\s*=>/)) {
        issues.push({
          ...
          recommendation:
            'Add cleanup function to useEffect if it sets up subscriptions/timers',
        });
      }
    }
    ...
    if (
      ...
      issues.push({
        ...
        recommendation:
          'Type event handlers: onClick: (e: React.MouseEvent) => void',
      });
    }
   ...
   */
  private reviewCodeStyle(code: string): {
    issues: QualityIssue[];
    strengths: string[];
  } {
    const issues: QualityIssue[] = [];
   ...
   * Assess testability
   */
  private assessTestability(code: string): {
    issues: QualityIssue[];
    strengths: string[];
  } {
    const issues: QualityIssue[] = [];
   ...
   * Identify security risks
   */
  private identifySecurityRisks(code: string): {
    issues: QualityIssue[];
    strengths: string[];
  } {
    const issues: QualityIssue[] = [];
  ...
  private calculateMetrics(
    ...
    const complexity = Math.min(
      10,
      Math.ceil((lines / 50 + hooksCount / 3 + nestingDepth / 5) / 3)
    );
    ...
    const testCoveragePotential =
      (hasExplicitProps ? 40 : 0) +
      (hasTestIds ? 30 : 0) +
  ...
  private generateRecommendations(
    ...
    if (blockers.length > 0) {
      recommendations.push(
        `🔴 BLOCKERS: Fix ${blockers.length} blocking issues before deployment`
      );
    ...
    if (critical.length > 0) {
      recommendations.push(
        `🟠 CRITICAL: Address ${critical.length} critical issues for production readiness`
      );
    }
    ...
    if (metrics.estimatedBundleSize > 30000) {
      recommendations.push(
        '⚡ PERFORMANCE: Consider code splitting to reduce bundle size'
      );
    }
    ...
    if (metrics.testCoveragePotential < 50) {
      recommendations.push(
        '🧪 TESTING: Improve testability by adding props interfaces and test IDs'
      );
    }
   ...
   */
  private generateSummary(score: number, approved: boolean, issueCount: number): string {
