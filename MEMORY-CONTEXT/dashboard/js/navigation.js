    ...
    async renderCheckpoints(id) {
        ...
        if (id) {
            ...
            try {
                ...
                if (!checkpoint) {
                    throw new Error('Checkpoint not found in data');
                }
                                ...
                                <div class="card-content" style="text-align: left !important;">
                                    <div style="max-height: 600px; overflow-y: auto; text-align: left !important;">
                                        ${cmds.slice(0, 50).map(cmd => {
                                            const submodule = this.extractSubmodule(cmd.checkpoint_id);
                                            const project = this.extractProject(cmd.checkpoint_id);
                                            return `<div style="padding: var(--space-3); margin: var(--space-2) 0; background: var(--bg-tertiary); border-radius: var(--radius-md); font-family: monospace; font-size: var(--text-sm); border-left: 3px solid var(--primary-500); text-align: left !important;"><div style="color: var(--text-primary); font-weight: var(--font-semibold); margin-bottom: var(--space-2); word-wrap: break-word; white-space: pre-wrap; overflow-wrap: break-word; text-align: left !important;">${this.escapeHtml(cmd.command_text)}</div><div style="color: var(--text-tertiary); font-size: var(--text-xs); word-wrap: break-word; overflow-wrap: break-word; text-align: left !important;">📅 ${new Date(cmd.timestamp).toLocaleString()}</div>${project ? `<div style="color: var(--text-tertiary); font-size: var(--text-xs); margin-top: var(--space-1); word-wrap: break-word; overflow-wrap: break-word; text-align: left !important;">📦 Project: ${this.escapeHtml(project)}</div>` : ''}${submodule ? `<div style="color: var(--text-tertiary); font-size: var(--text-xs); margin-top: var(--space-1); word-wrap: break-word; overflow-wrap: break-word; text-align: left !important;">📂 Submodule: ${this.escapeHtml(submodule)}</div>` : ''}<div style="color: var(--text-tertiary); font-size: var(--text-xs); margin-top: var(--space-1); word-wrap: break-word; overflow-wrap: break-word; text-align: left !important;">🔖 Session: ${this.escapeHtml(cmd.checkpoint_id.substring(0, 60))}${cmd.checkpoint_id.length > 60 ? '...' : ''}</div></div>`;
                                        }).join('')}
                                        ${cmds.length > 50 ? `
